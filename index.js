...
function finalScore(inningCB, desiredInnings) {
  let homeTeam = 0;
  let awayTeam = 0;
  for (let i = 0; i < desiredInnings; i++) {
    homeTeam += inningCB();
    awayTeam += inningCB();
  }
...
function scoreboard(getInningScoreCB, inningCB, desiredInnings) {
  let result = [];
  let homeSum = 0;
  let awaySum = 0;
  for (let i = 0; i < desiredInnings; i++) {
  ...
  if (homeSum === awaySum) {
    ...
    result.push(`Final Score: Away ${awaySum} - Home ${homeSum}`);
  }
  return result;
}
